...
using OpenMcdf;
    ...
    public class Program
    {
        ...
        static void TestDecryption(string encryptedPath, string password, string label)
        {
            try
            {
                Console.WriteLine($"\n{label} を復号化中...");
                byte[] decrypted = ExcelEncryptor.Decrypt(encryptedPath, password);

                Console.WriteLine($"  復号化後サイズ: {decrypted.Length} bytes");
                Console.WriteLine($"  最初の16バイト: {BitConverter.ToString(decrypted, 0, Math.Min(16, decrypted.Length)).Replace("-", " ")}");
        ...
        static void CompareWithOriginal(string originalPath, string dotnetPath, string poiPath, string password)
        {
            try
            {
                byte[] original = File.ReadAllBytes(originalPath);
                byte[] dotnetDecrypted = ExcelEncryptor.Decrypt(dotnetPath, password);
                byte[] poiDecrypted = ExcelEncryptor.Decrypt(poiPath, password);
    ...
    public class ExcelEncryptor
    {
        ...
        public static void Encrypt(string inputPath, string outputPath, string password)
        {
            ...
            byte[] encryptedPackage = EncryptPackage(packageData, encryptionKey, keySalt);
        ...
        {
            byte[] keySalt = RandomBytes(SALT_SIZE);      // keyData.saltValue
            byte[] verifierSalt = RandomBytes(SALT_SIZE);  // p:encryptedKey.saltValue
            byte[] pwHash = HashPassword(password, verifierSalt, SPIN_COUNT);

            // 検証データ（verifier / verifierHash）
            byte[] verifier = RandomBytes(SALT_SIZE);
            byte[] keySpec  = RandomBytes(KEY_SIZE / 8);   // 実際の AES 鍵 (encryptedKey の中身)
            ...
            byte[] encryptedVerifier = HashInput(pwHash, verifierSalt, kVerifierInputBlock, verifier, KEY_SIZE / 8);
            byte[] verifierHash = SHA1.Create().ComputeHash(verifier);
            byte[] encryptedVerifierHash = HashInput(pwHash, verifierSalt, kHashedVerifierBlock, verifierHash, KEY_SIZE / 8);
            byte[] encryptedKey = HashInput(pwHash, verifierSalt, kCryptoKeyBlock, keySpec, KEY_SIZE / 8);

            // dataIntegrity: encryptedHmacKey だけ先に作る
            byte[] integritySalt = RandomBytes(HASH_SIZE);              // HMAC の生鍵（20B）
            byte[] kIntegrityKeyBlock = { 0x5F, 0xB2, 0xAD, 0x01, 0x0C, 0xB9, 0xE1, 0xF6 };
            byte[] ivKey = GenerateIv(keySalt, kIntegrityKeyBlock, BLOCK_SIZE);
        ...
        private static void UpdateIntegrityHMAC(byte[] encryptedPackage, int oleStreamSize, byte[] encryptionKey, byte[] keySalt, byte[] integritySalt, XDocument xmlDoc)
        {
            using (var hmac = new HMACSHA1(integritySalt))
            {
                ...
                byte[] ivVal = GenerateIv(keySalt, kIntegrityValueBlock, BLOCK_SIZE);
            ...
            using (var root = RootStorage.OpenRead(encryptedPath))
            {
                ...
                using (var reader = new BinaryReader(encInfoStream))
                {
                    ...
                    // XMLからsalt等を抽出（簡易パーサー）
                      var keySaltMatch = System.Text.RegularExpressions.Regex.Match(xmlString, @"<keyData[^>]*saltValue=""([^""]+)""");
                      var verifierSaltMatch = System.Text.RegularExpressions.Regex.Match(xmlString, @"<p:encryptedKey[^>]*saltValue=""([^""]+)""");
                      var spinCountMatch = System.Text.RegularExpressions.Regex.Match(xmlString, @"spinCount=""(\d+)""");
                      var encryptedKeyMatch = System.Text.RegularExpressions.Regex.Match(xmlString, @"encryptedKeyValue=""([^""]+)""");


                    if (!keySaltMatch.Success || !verifierSaltMatch.Success || !spinCountMatch.Success || !encryptedKeyMatch.Success)
                        throw new Exception("暗号化情報の解析に失敗");
                    ...
                    byte[] kCryptoKeyBlock = { 0x14, 0x6e, 0x0b, 0xe7, 0xab, 0xac, 0xd0, 0xd6 };
                    byte[] intermedKey = GenerateKey(pwHash, kCryptoKeyBlock, KEY_SIZE / 8);
                    byte[] iv = GenerateIv(verifierSalt, null, BLOCK_SIZE);
                    ...
                    byte[] actualKey = new byte[KEY_SIZE / 8];
                    ...
                    using (var br = new BinaryReader(encPackageStream))
                    {
                        ...
                        using (var ms = new MemoryStream())
                        {
                            ...
                            while (remaining > 0)
                            {
                                int segSize = (int)Math.Min(SEGMENT_LENGTH, remaining);
                                bool isLast = (remaining <= SEGMENT_LENGTH);

                                byte[] encryptedSeg = isLast ? br.ReadBytes(PadLen((int)remaining)) : br.ReadBytes(SEGMENT_LENGTH);
                                byte[] blockKey = BitConverter.GetBytes(block);
                                byte[] segIv = GenerateIv(keySalt, blockKey, BLOCK_SIZE);
        ...
        private static byte[] EncryptPackage(byte[] packageData, byte[] encryptionKey, byte[] keySalt)
        {
            ...
            using (var bw = new BinaryWriter(ms))
            {
                ...
                int offset = 0; int block = 0;
                while (offset < packageData.Length)
                {
                    int segSize = Math.Min(SEGMENT_LENGTH, packageData.Length - offset);
                    bool isLast = (offset + segSize >= packageData.Length);
                    byte[] seg = new byte[segSize];
                    Array.Copy(packageData, offset, seg, 0, segSize);
                    byte[] blockKey = BitConverter.GetBytes(block);
                    byte[] iv = GenerateIv(keySalt, blockKey, BLOCK_SIZE);
                    using (var aes = Aes.Create())
                    {
                        aes.Key = encryptionKey; aes.IV = iv; aes.Mode = CipherMode.CBC;
                        aes.Padding = isLast ? PaddingMode.PKCS7 : PaddingMode.None;
                        if (!isLast && segSize < SEGMENT_LENGTH)
                        {
                            byte[] padSeg = new byte[SEGMENT_LENGTH];
        ...
        private static void CreateEncryptedFile(string outputPath, XDocument encryptionInfo, byte[] encryptedData)
        {
            ...
            {
                using (var s = root.CreateStream("EncryptedPackage")) s.Write(encryptedData, 0, encryptedData.Length);
                CreateDataSpacesStructure(root);
                using (var s2 = root.CreateStream("EncryptionInfo"))
                using (var bw = new BinaryWriter(s2))
                {
                    bw.Write((ushort)4); bw.Write((ushort)4); bw.Write((uint)0x40);
                    string xml = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>" + encryptionInfo.Root.ToString(SaveOptions.DisableFormatting);
                    xml = xml.Replace(" />", "/>");
        ...
        private static void CreateDataSpacesStructure(RootStorage root)
        {
            ...
            using (var v = ds.CreateStream("Version")) using (var bw = new BinaryWriter(v))
            {
                WriteUnicodeLPP4(bw, "Microsoft.Container.DataSpaces"); bw.Write((ushort)1); bw.Write((ushort)0); bw.Write((ushort)1); bw.Write((ushort)0); bw.Write((ushort)1); bw.Write((ushort)0);
            }
            using (var m = ds.CreateStream("DataSpaceMap")) using (var bw = new BinaryWriter(m))
            {
                bw.Write((uint)8); bw.Write((uint)1);
                long pos = m.Position; bw.Write((uint)0);
                bw.Write((uint)1); bw.Write((uint)0); WriteUnicodeLPP4(bw, "EncryptedPackage"); WriteUnicodeLPP4(bw, "StrongEncryptionDataSpace");
                long end = m.Position; m.Seek(pos, SeekOrigin.Begin); bw.Write((uint)(end - pos)); m.Seek(end, SeekOrigin.Begin);
            }
            var dsi = ds.CreateStorage("DataSpaceInfo");
            using (var s = dsi.CreateStream("StrongEncryptionDataSpace")) using (var bw = new BinaryWriter(s))
            { bw.Write((uint)8); bw.Write((uint)1); WriteUnicodeLPP4(bw, "StrongEncryptionTransform"); }
            var ti = ds.CreateStorage("TransformInfo");
            var st = ti.CreateStorage("StrongEncryptionTransform");
            using (var p = st.CreateStream("\u0006Primary")) using (var bw = new BinaryWriter(p))
            {
                long hdr = p.Position; bw.Write((uint)0); bw.Write((uint)1); WriteUnicodeLPP4(bw, "{FF9A3F03-56EF-4613-BDD5-5A41C1D07246}");
                long hdrEnd = p.Position; p.Seek(hdr, SeekOrigin.Begin); bw.Write((uint)(hdrEnd - hdr)); p.Seek(hdrEnd, SeekOrigin.Begin);
                WriteUnicodeLPP4(bw, "Microsoft.Container.EncryptionTransform");
                bw.Write((ushort)1); bw.Write((ushort)0); bw.Write((ushort)1); bw.Write((ushort)0); bw.Write((ushort)1); bw.Write((ushort)0);
                bw.Write((uint)0); bw.Write((uint)0); bw.Write((uint)0); bw.Write((uint)4);
            }
        }
        ...
        private static byte[] PadBlock(byte[] d) { int s = PadLen(d.Length); var r = new byte[s]; Buffer.BlockCopy(d,0,r,0,d.Length); return r; }
        private static int PadLen(int len) { return ((len+15)/16)*16; }
        private static byte[] HashPassword(string pw, byte[] salt, int spin) { var pwb = Encoding.Unicode.GetBytes(pw); using (var sha = SHA1.Create()){ sha.TransformBlock(salt,0,salt.Length,null,0); sha.TransformFinalBlock(pwb,0,pwb.Length); var h=sha.Hash; for(int i=0;i<spin;i++){ sha.Initialize(); var iter=BitConverter.GetBytes(i); sha.TransformBlock(iter,0,4,null,0); sha.TransformFinalBlock(h,0,h.Length); h=sha.Hash;} return h;} }
        private static byte[] HashInput(byte[] pwHash, byte[] salt, byte[] blk, byte[] input, int keySize) { var k = GenerateKey(pwHash, blk, keySize); var iv = GenerateIv(salt, null, BLOCK_SIZE); var pad = PadBlock(input); return EncryptWithAES(pad,k,iv);}
        private static byte[] GenerateKey(byte[] h, byte[] blk, int ks) { using (var sha=SHA1.Create()){ sha.TransformBlock(h,0,h.Length,null,0); sha.TransformFinalBlock(blk,0,blk.Length); var d=sha.Hash; var k=new byte[ks]; Array.Copy(d,k,Math.Min(d.Length,ks)); return k;} }
        private static byte[] GenerateIv(byte[] salt, byte[] blk, int bs) { if(blk==null){ var iv=new byte[bs]; Array.Copy(salt,iv,Math.Min(salt.Length,bs)); return iv;} using(var sha=SHA1.Create()){ sha.TransformBlock(salt,0,salt.Length,null,0); sha.TransformFinalBlock(blk,0,blk.Length); var d=sha.Hash; var iv=new byte[bs]; Array.Copy(d,iv,Math.Min(d.Length,bs)); return iv;} }
        private static byte[] EncryptWithAES(byte[] d, byte[] k, byte[] iv) { using(var aes=Aes.Create()){ aes.Key=k; aes.IV=iv; aes.Mode=CipherMode.CBC; aes.Padding=PaddingMode.None; return aes.CreateEncryptor().TransformFinalBlock(d,0,d.Length);} }
